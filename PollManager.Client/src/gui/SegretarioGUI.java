...
if (range3Text.getText().length() == 0) {
	...
	private void salvaMultiplaActionPerformed(java.awt.event.ActionEvent evt) {
		...
		try {
			...
			creaRispostaMultipla(opzione1,0);

			if (!opzione2.isEmpty()) {

				creaRispostaMultipla(opzione2,1);

				if (!opzione3.isEmpty()) {
					creaRispostaMultipla(opzione3,2);

					if (!opzione4.isEmpty()) {
						
						creaRispostaMultipla(opzione4,3);
						
						if (!opzione5.isEmpty()) {
							
							creaRispostaMultipla(opzione5,5);
							
							if (!opzione6.isEmpty()) {
								
								creaRispostaMultipla(opzione6,6);
							}
						}
					}
				}
	 ...
	 */
	private void creaRispostaMultipla(String opzione,int ordine) {
		RispostaDTO risposta = new RispostaDTO();

		risposta.setDomandaAssociataIdDomanda(idDomanda);
		risposta.setHasTestoLibero(false); //è "il specificare...."
